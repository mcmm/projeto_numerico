...
for i in range(1,n+1):
    ...
    funcao_primeiro_intervalo = lambda x: (((x-x_i[i-1])/h))*(1500*(e**(((-1)*(x-(L/2)))/(sigma**2))) - 397*(e**(((-1)*(x**2))/(teta**2))-(e**(((-1)*(x-L))/(sigma**2))))) #mudar dps para funcao do calor
    funcao_segundo_intervalo = lambda x: (((x_i[i+1]-x)/h))*(1500*(e**(((-1)*(x-(L/2)))/(sigma**2))) - 397*(e**(((-1)*(x**2))/(teta**2))-(e**(((-1)*(x-L))/(sigma**2))))) #mudar dps para funcao do calor
